...
function Dashboard() {
  const [userData, setUserData] = useState({ stage: "", name: "", health: "", page: ""});
  ...
  const getUserData = async () => {
    ...
    if (data) {
      setUserData({ stage: data.stage, name: data.name, health: data.health, page: data.page});
      console.log(data);
    } else {
      console.log(error);
    }
...
const handleSubmitNext = async () => {
    try {
        ...
        const { data, error } = await supabase
            .from("Game")
            .update({ page: userData.page + 1})
            .eq("email", emailId)
            .single();
        
        if (error) {
            console.log("Error updating page number: ", error.message);
        } else {
            console.log("Page number incremented successfully!", data);
        }
    } catch (error) {
      ...
      <div className="ui grid">
        <div className="four wide column">
          <div className="ui labeled icon vertical menu">
            ...
            <a className="item">
              <i className="suitcase icon"></i> Backpack
            </a>
            <a className="item">
              <i className="heartbeat icon"></i> Health
            </a>
            <a className="item">
              <i className="map icon"></i> Map
            </a>
            <a className="item">
              <i className="book icon"></i> Dossier
            </a>
          </div>
        </div>
